#!/usr/bin/env python3
"""
Print a summary of typo test results from the results/ directory.

Shows per-agent accuracy broken down by tool, plus failed cases.

Usage:
    python -m tests.test_typos_in_messages.print_results
"""
...
def assign_tool(index: int) -> str:
    return TOOLS[index // CASES_PER_TOOL]
...
def print_agent_summary(agent_name: str, results: list[dict]):
    ...
    if failures:
        print(f"\n    Failed cases:")
        for r in failures:
            print(f"      [{r['tool_group']:25s}] expected={r['expected']:25s} predicted={r['predicted']:25s}  \"{r['message'][:60]}\"")
...
def main():
    ...
    print(f"{'=' * 70}")
    print(f"  Typo Resilience Test Results ({RESULTS_DIR})")
    print(f"{'=' * 70}")
