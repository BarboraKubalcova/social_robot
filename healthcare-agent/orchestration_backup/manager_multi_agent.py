...
class AppointmentAgent:
    ...
    def __init__(self, llm: OllamaClient, appointments: AppointmentManager):
        self.llm = llm
    ...
    def _execute_action(
        ...
        if action == "list_slots":
            if not available_slots:
                return {"success": True, "report": "There are no available slots in the current timetable."}
            preview = ", ".join(
                f"{s['id']} ({s.get('day', '')} {s.get('date', '')} {s.get('time', '')})"
                for s in available_slots[:10]
            )
            return {"success": True, "report": f"Available slots: {preview}"}

        if action == "list_appointments":
            if not booked:
                return {"success": True, "report": "You do not have any booked appointments."}
            preview = ", ".join(
                f"{a['id']} ({a.get('day', '')} {a.get('date', '')} {a.get('time', '')})"
                for a in booked[:10]
            )
            return {"success": True, "report": f"Your booked appointments: {preview}"}
        ...
        if action == "book":
            ...
            if ok:
                return {"success": True, "report": f"Successfully booked appointment at {slot_id}."}
            return {"success": False, "report": f"Could not book {slot_id}. It may already be taken or invalid."}
        ...
        if action == "cancel":
            ...
            if ok:
                return {"success": True, "report": f"Appointment {appt_id} has been canceled."}
            return {"success": False, "report": f"Could not cancel appointment {appt_id}."}
        ...
        if action == "reschedule":
            ...
            cancel_ok = self.appointments.cancel_appointment(appt_id)
            if not cancel_ok:
                return {"success": False, "report": f"Could not cancel the old appointment {appt_id}."}
            book_ok = self.appointments.book_appointment(new_slot)
            if not book_ok:
                return {"success": False, "report": f"Canceled {appt_id} but could not book {new_slot}. Manual review needed."}
            return {"success": True, "report": f"Appointment {appt_id} rescheduled to {new_slot}."}
...
class MedicalKnowledgeAgent:
    ...
    def execute(self, task: str, history_text: str) -> Dict[str, Any]:
        ...
        context = prompt_kwargs.get("context", "No relevant documents found.")
...
class MessagingAgent:
    ...
    def __init__(self, llm: OllamaClient, messaging: MessageManager):
        self.llm = llm
...
class MultiAgentManager:
    ...
    def __init__(self) -> None:
        ...
        self.agents: Dict[str, Any] = {
            "appointment_agent": AppointmentAgent(self.llm, self.appointments),
            "medical_knowledge_agent": MedicalKnowledgeAgent(self.llm, self.retriever),
            "messaging_agent": MessagingAgent(self.llm, self.messaging),
